...
const calculateProductSliderMargin = ({
  ...
  const nbrProductTileFitScreen = Math.floor(
    sliderWidth / (productTileWidth + sliderGapSize),
  );
  return (
    sliderWidth -
    nbrProductTileFitScreen * productTileWidth -
    (nbrProductTileFitScreen - 1) * sliderGapSize
  );
};
