...
axiosRetry(axiosInstance, {
  ...
  onRetry: async (retryCount, error, requestConfig) => {
    console.log(`Retry attempt ${retryCount}: ${requestConfig.method?.toUpperCase()} ${requestConfig.url}`, {
      error,
    })
...
const axiosClient = {
  ...
  post: async <T, D = unknown>(url: string, data: D, config: CustomAxiosConfig = {}): Promise<T> => {
    try {
      const headers = await resolveHeaders(config)
      const requestConfig = { ...config, headers }
      const resp: AxiosResponse<T> = await axiosInstance.post(url, data, requestConfig)
      return resp.data
    } catch (error) {
  ...
  put: async <T, D = unknown>(url: string, data: D, config: CustomAxiosConfig = {}): Promise<T> => {
    try {
      const headers = await resolveHeaders(config)
      const requestConfig = { ...config, headers }
      const resp: AxiosResponse<T> = await axiosInstance.put(url, data, requestConfig)
      return resp.data
    } catch (error) {
      throw new HttpClientError(error as Error, 'POST', url, config.traceId)
    }
  ...
  delete: async <T>(url: string, config: CustomAxiosConfig = {}): Promise<T> => {
    try {
      const headers = await resolveHeaders(config)
      const requestConfig = { ...config, headers }
      const resp: AxiosResponse<T> = await axiosInstance.delete(url, requestConfig)
      return resp.data
    } catch (error) {
