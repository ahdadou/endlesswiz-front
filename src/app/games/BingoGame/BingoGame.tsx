...
const BingoGame = ({
  ...
  const callDefinition = () => {
    const remaining = words.filter(
      (w) => !calledDefinitions.includes(w.meaning),
    );
    const randomDef =
      remaining[Math.floor(Math.random() * remaining.length)].meaning;
          ...
          row.map((word, j) => (
            <div
              ...
              className={`aspect-square flex items-center justify-center text-center p-2 border-2 rounded-lg cursor-pointer
                ${
                  calledDefinitions.includes(
                    words.find((w) => w.word === word)?.meaning || "",
                  )
